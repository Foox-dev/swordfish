...
static void tui_render_list(tui_state_t *s) {
	...
	for (int i = 0; i < height && (s->scroll + i) < s->row_count; ++i) {
		...
		int pid_pair = is_root
			? (is_cursor ? PAIR_PID_ROOT_HIGHLIGHT : (is_selected ? PAIR_PID_ROOT_SELECTED : PAIR_PID))
			: (is_cursor ? PAIR_PID_HIGHLIGHT : (is_selected ? PAIR_PID_SELECTED : PAIR_PID));
		...
		int user_pair = is_root
			? (is_cursor ? PAIR_USER_ROOT_HIGHLIGHT : (is_selected ? PAIR_USER_ROOT_SELECTED : PAIR_USER))
			: (is_cursor ? PAIR_USER_HIGHLIGHT : (is_selected ? PAIR_USER_SELECTED : PAIR_USER));
		...
		int state_pair = is_root
			? (is_cursor ? PAIR_STATE_ROOT_HIGHLIGHT : (is_selected ? PAIR_STATE_ROOT_SELECTED : PAIR_STATE))
			: (is_cursor ? PAIR_STATE_HIGHLIGHT : (is_selected ? PAIR_STATE_SELECTED : PAIR_STATE));
		...
		int ram_pair = is_root
			? (is_cursor ? PAIR_RAM_ROOT_HIGHLIGHT : (is_selected ? PAIR_RAM_ROOT_SELECTED : PAIR_RAM))
			: (is_cursor ? PAIR_RAM_HIGHLIGHT : (is_selected ? PAIR_RAM_SELECTED : PAIR_RAM));
...
static void tui_render_status(tui_state_t *s) {
	...
	mvwprintw(w, 0, 0, "  %d/%d processes    %d selected",
	          s->row_count, s->proc_count, s->selected_count);
...
static void tui_handle_input(tui_state_t *s, int ch) {
	...
	case 4: tui_cursor_move(s, +tui_list_height() / 2); break;
	case 21: tui_cursor_move(s, -tui_list_height() / 2); break;
