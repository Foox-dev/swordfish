...
static void compile_patterns(const swordfish_args_t *args, pattern_list_t *plist, compiled_pattern_t *compiled);
static bool match_process(const char *name, const char *cmdline, compiled_pattern_t *compiled, int pattern_count);
static void free_compiled_patterns(compiled_pattern_t *compiled, int count);
static bool entry_matches(const process_info_t *p, pattern_list_t *plist, const swordfish_args_t *args, compiled_pattern_t *compiled);
static void fmt_ram(char *buf, size_t len, long ram_mb);
static void print_proc_info(const process_info_t *p, int sig, const swordfish_args_t *args, const char *prefix, bool include_signal);
...
static int find_matching_processes(const swordfish_args_t *args, pattern_list_t *plist, process_info_t *matches, compiled_pattern_t *compiled);
static void select_processes(int matched, process_info_t *matches, int *selected, int *count, const swordfish_args_t *args, int sig);
static int confirm_and_act(const swordfish_args_t *args, int count, int *selected, process_info_t *matches);
...
/* Check if any selected process is owned by root */
static bool has_root_process(int count, int *selected, process_info_t *matches, int matched_total) {
	if (count > 0 && selected) {
		for (int i = 0; i < count; ++i) {
...
static int find_matching_processes(const swordfish_args_t *args, pattern_list_t *plist,
	...
	while ((entry = readdir(proc))) {
		...
		if (args->operation == SWOP_FUZZY || !entry_matches(&p, plist, args, compiled)) {
			...
			if (fcmd) {
				size_t nr = fread(p.cmdline, 1, sizeof(p.cmdline) - 1, fcmd);
				fclose(fcmd);
...
/* Interactive process selection for "-S" */
...
/* Confirm and act on selected processes */
static int confirm_and_act(const swordfish_args_t *args, int count, int *selected,
	...
	if (!args->auto_confirm && is_interactive()) {
		for (int i = 0; i < count; ++i)
