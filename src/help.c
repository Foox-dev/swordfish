...
const swordfish_option_t swordfish_options[] = {
	{"-k", NULL, "Send signal to matching processes (default: SIGTERM)", "operation", true},
	{"-S", NULL, "Interactively select which processes to act on", "operation", true},
	{"-F", NULL, "Interactively select processes with a fuzzy finder", "operation", true},
	{"-x", NULL, "Exact match process names (default: substring match)", "modifier", true},
	{"-y", NULL, "Auto-confirm; skip all prompts", "modifier", true},
	{"-v", NULL, "Increase verbosity level up to -vvv for maximum verbosity", "modifier", true},
	{"-t", NULL, "Always act on only the top matched process", "modifier", true},
	{"-p", NULL, "Print raw PIDs only", "modifier", true},
	{"-n", NULL, "Dry run; show what would happen without doing it", "modifier", true},
	{"-w", NULL, "Wait for the process to die after sending signal", "modifier", true},
	{"-r", NULL, "Hide processes owned by root", "modifier", true},
};
...
const swordfish_option_t swordfish_options_lopt[] = {
	{"--user", "<user>", "Filter processes by username", "filter", false},
	{"--exclude", "<pattern>", "Exclude processes matching pattern", "filter", false},
	{"--sort", "<ram|age>", "Sort process list by RAM or age", "filter", false},
	{"--parent", "<pid>", "Match only children of the given parent PID", "filter", false},
	{"--session", "<sid>", "Match processes by session ID", "filter", false},
	{"--pidfile", "<file>", "Read target PID from file", "filter", false},
	{"--retry", "<seconds>", "Retry every <seconds> if no match found", "behavior", false},
	{"--timeout", "<seconds>", "Escalate to SIGKILL after <seconds> if process does not die", "behavior", false},
	{"--pre-hook", "<script>", "Run <script> before sending signals", "hook", false},
	{"--post-hook", "<script>", "Run <script> after sending signals", "hook", false},
	{"--completions", "<shell>", "Generate shell completions for fish, bash, or zsh", "misc", false},
	{"--man", "[file]", "Generate man page, optionally writing to <file>", "misc", false},
	{"--version", NULL, "Show installed version", "misc", false},
	{"--help", "[category]", "Show help, optionally for a specific category", "misc", false},
};
...
/* Prints full help, optionally for a specific category */
void help(const char *category) {
	...
	if (category) {
		...
		if (strcmp(category, "arguments") == 0) {
...
static void man_args(FILE *out, bool isman) {
	...
	static const char *categories[] = {"filter", "behavior", "hook", "misc", NULL};
	...
	for (int ci = 0; categories[ci]; ci++) {
		...
		for (size_t i = 0; i < swordfish_options_lopt_count; ++i) {
			if (strcmp(swordfish_options_lopt[i].category, categories[ci]) != 0) continue;
			char buf[64] = {0};
